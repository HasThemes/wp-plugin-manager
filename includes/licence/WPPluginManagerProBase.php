	...
	class WPPluginManagerProBase {
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_wp-plugin-manager-pro_fupc', function(){
						...
						delete_transient( 'htpmpro_update_checked_' . $this->product_id );
						delete_transient( 'htpmpro_update_lock_' . $this->product_id );
						delete_transient( 'htpmpro_activation_lock_' . $this->product_id );
		...
		public function isWooLentorBundle(){
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","htpmpro").'</h1>');
		...
		function el_plugin_update_info(){
			$update_checked = 'htpmpro_update_checked_' . $this->product_id;
			$response = get_transient( $update_checked );
			if ( $response ) {
				set_transient( $update_checked, true, DAY_IN_SECONDS * 7 );
				return;
			}
		...
		function plugin_update($transient)
		{
			...
			$updated_lock_key = 'htpmpro_update_lock_' . $this->product_id;

			// Check if the transient exists
			$response = get_transient($updated_lock_key);
			if ( false === $response ) {
				$response = $this->el_plugin_update_info();
				set_transient($updated_lock_key, $response, DAY_IN_SECONDS * 7);
			}
		...
		final function _check_wp_plugin( $purchase_key, &$error = "", &$response_obj = null ) {
			...
			$activation_lock_key = 'htpmpro_activation_lock_' . $this->product_base;
			...
			// Check if the transient exists
			$response = get_transient($activation_lock_key);
			if ( false === $response ) {
				$response = $this->_request( 'product/active/'.$this->product_id, $param, $error );
				set_transient($activation_lock_key, $response, DAY_IN_SECONDS * 7);
			}
                ...
                if ( empty( $response->code ) ) {
                    if ( ! empty( $response->status ) ) {
                        if ( ! empty( $response->data ) ) {
                           ...
                           // $license_obj = unserialize( $serial_obj ); // old method
                            if ( $license_obj &&  $license_obj->is_valid ) {
